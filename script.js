...
function addSheetTabEventListeners() {
    $(".sheet-tab.selected").bind("contextmenu", function (e) {
        ...
        let modal = $(`<div class="sheet-options-modal">
                        <div class="option option-1 sheet-rename">Rename</div>
                        <div class="option option-2 sheet-delete">Delete</div>
                    </div>`);
...
function openFile() {
    ...
    inputFile.change(function (e) {
        ...
        reader.onload = function () {//jb data read hojayegaa uske baad chlegaa yeh function
            emptySheet();
            $(".sheet-tab").remove();
...
function updateStreams(ele, elements, update, oldUpstream) {
    ...
    //validation between downstrem and upcoming upstream
    if(cellData[selectedSheet][rowId - 1] && cellData[selectedSheet][rowId - 1][colId - 1]) {
    ...
    if (!cellData[selectedSheet][rowId - 1]) {
        ...
        if(update) {
            for(let i of oldUpstream) {
                let [calRowId, calColId] = calcSelfValue(i);
                let index = cellData[selectedSheet][calRowId - 1][calColId - 1].downStream.indexOf(selfName);
    ...
    for(let i of elements) {
        ...
        if(!cellData[selectedSheet][calRowId - 1]) {
            ...
            cellData[selectSheet][calRowId - 1][calColId - 1] = {...defaultProperties, "upstream" : [], "downStream" : [selfName]};
        }
        else if(!cellData[selectedSheet][calRowId - 1][calColId - 1]) {
            cellData[selectedSheet][calRowId - 1][calColId - 1] = {...defaultProperties, "upStream": [], "downStream":[selfName]};
        }
        else {
